...
namespace FrbaOfertas.ConectorDB
{
    class FuncionesProveedor
    {
        ...
        public static Boolean existeCUIT(string cuit)
        {
            return FrbaOfertas.ConectorDB.FuncionesGlobales.existeTabla(cuit, "CUIT");

        }

        public static Boolean existeRS(string RS)
        {
            return FrbaOfertas.ConectorDB.FuncionesGlobales.existeTabla(RS, "RS");
        ...
        public static void LoguearUsuarioProveedor(int id)
        {
            SqlConnection connection = new SqlConnection(Conexion.getStringConnection());
            SqlCommand comm = connection.CreateCommand();
            comm.CommandText = "SELECT usuario_id FROM HPBC.Usuario join HPBC.Proveedor on usuario_id = Provee_usuario_id where Provee_ID = " + id;
            comm.Connection = connection;
            comm.Connection.Open();
            SqlDataReader reader = comm.ExecuteReader() as SqlDataReader;
            while (reader.Read())
